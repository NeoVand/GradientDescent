<script lang="ts">
  ...
  const modelFormulas: Record<string, string> = {
    ...
    'rotated-valley': String.raw`\text{pure surface}`,
  ...
  const lossFormulas: Record<string, string> = {
    ...
    'rotated-valley': String.raw`\mathcal{L} = 1.1\,\alpha^2 + 1.8\,\alpha\beta + 1.1\,\beta^2 \quad (\lambda = 4,\, 0.4\ \text{at}\ 45^\circ;\; \kappa = 10)`,
  ...
  const gradientFormulas: Record<string, string> = {
    ...
    'rotated-valley': String.raw`\nabla \mathcal{L} = \begin{bmatrix} 2.2\,\alpha + 1.8\,\beta & 1.8\,\alpha + 2.2\,\beta \end{bmatrix}^\top`,
